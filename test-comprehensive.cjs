...
async function testSearch(query, encoded) {
  const s = await req('/api/hunter/search?q=' + encoded);
  console.log(`\n--- Search: "${query}" ---`);
  console.log('Results count:', s.results?.length || 0);
...
async function testLookup(id, name) {
  ...
  console.log('eBay sold count:', l.consolidated?.recentSoldCount || 0);
  ...
  // Check URL presence
  const ebayWithUrl = l.prices?.filter(p => p.source === 'ebay' && p.url);
  console.log('eBay prices with URLs:', ebayWithUrl?.length || 0);
...
async function main() {
  ...
  allPass &= await testSearch('psyduck 104/147 reverse holo', 'psyduck+104%2F147+reverse+holo');
  allPass &= await testSearch('charizard 4/102 base set shadowless', 'charizard+4%2F102+base+set+shadowless');
  allPass &= await testSearch('2001 lugia game boy promo psa 10', '2001+lugia+game+boy+promo+psa+10');
