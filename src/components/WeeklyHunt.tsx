...
export default function WeeklyHunt() {
  ...
  const picks = top.filter((r) => r.passes_filter);
  ...
  if (error) {
    return (
      <div
        className="card"
        style={{ textAlign: "center", padding: 24, marginTop: 12, color: "var(--clay)", border: "1px solid var(--clay)" }}
      >
        {error}
      </div>
    );
  ...
  function gradeCell(row: HuntRow) {
    if (row.passes_filter) {
      return (
        <span style={{ color: "var(--sage)", fontWeight: 700 }}>
          {row.margin.toFixed(1)}x
        </span>
      );
    }
    return <span style={{ color: "var(--text-mid)" }}>{row.margin.toFixed(1)}x</span>;
  }
  ...
  return (
    <div style={{ marginTop: 16 }}>
      {picks.length > 0 && (
        <div
          className="card"
          style={{ marginBottom: 12, background: "rgba(73,184,113,0.06)", border: "1px solid var(--sage)" }}
        >
            ...
            {top.map((r) => (
              ...
              >
                ...
                <td style={{ padding: "8px 12px", textAlign: "right", fontWeight: 500 }}>
                  ${r.raw_price.toFixed(0)}
                </td>
                <td style={{ padding: "8px 12px", textAlign: "right", color: "var(--text-mid)", fontSize: 12 }}>
                  {psaCell(r.psa6_price)}
                </td>
                <td style={{ padding: "8px 12px", textAlign: "right", color: "var(--text-mid)", fontSize: 12 }}>
                  {psaCell(r.psa7_price)}
                </td>
                <td style={{ padding: "8px 12px", textAlign: "right", color: "var(--text-mid)", fontSize: 12 }}>
                  {psaCell(r.psa8_price)}
                </td>
                <td style={{ padding: "8px 12px", textAlign: "right", color: "var(--text-mid)", fontSize: 12 }}>
                  {psaCell(r.psa9_price)}
                </td>
      ...
      >
        Week of {rows[0]?.week_start_date}
      </div>
    </div>
  );
}
